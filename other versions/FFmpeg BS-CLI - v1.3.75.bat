@echo off
title FFmpeg Buttery Smooth CLI (v1.3.75)
color 0B
set inputnumber=1
set islinkcopied=2
...
IF "%queueEnable%"=="yes" (
    ...
    echo.
      IF "%ffmpegBarEnable%"=="yes" (
            ...
            echo FFMS2("input.mp4"^) >> avsscript.avs
            echo ClipBlend(delay=%framesAmount%^) >> avsscript.avs
            echo changefps(60, linear=false^) >> avsscript.avs
            
            ffmpeg -i avsscript.avs %ffmpegstats% input.mp4 %gpu% -crf %crfValue% -c:a copy output-%fpsAmount%fps-resampled.mp4

            DEL /F /Q avsscript.avs
            DEL /F /Q input.mp4.ffindex
        )
)
...
:NoColorFix
set colorfix=colorspace=bt709:iall=smpte170m:fast=1
...
IF "%statsEnable%"=="yes" (
    ...
    set ffmpegstats=-v quiet -stats -i
      ...
      IF "%ffmpegBarEnable%"=="yes" (
            ffmpeg-bar -i input.mp4 %gpuUpscale% -vf %colorfix%,zscale=%widthupscale%:%heightupscale%:f=%upscalingmethod% -c:a copy Output-Upscale.mp4
        ) else (
            ffmpeg %ffmpegstats% input.mp4 %gpuUpscale% -vf %colorfix%,zscale=%widthupscale%:%heightupscale%:f=%upscalingmethod% -c:a copy Output-Upscale.mp4
        )
